...
function setHTML() {
  ...
  return '<div class="shead"><button class="iconbtn" data-act="close"><i data-lucide="arrow-left" class="ic"></i></button><h2>Настройки</h2></div>' +
    ...
    row('sparkle', 'Добавить демо-данные', 'полгода операций', 'demo-seed') +
    '</div>' +

    '<div class="label">AI-ассистент</div><div class="list">' +
    row('sparkle', 'AI-ассистент', st.ai && st.ai.key ? '' : 'офлайн-режим', 'ai') +
    '</div>' +

    '<div class="label">Приложение</div><div class="list">' +
    row('trash', 'Сбросить все данные', '', 'reset') +
    '</div>' +
    '</div>';
}
function setMount(el) {
  ...
  el.querySelectorAll('[data-tg]').forEach(b => b.onclick = () => {
    ...
    b.setAttribute('aria-checked', st[k]); haptic('selection'); render(true);
  });
  const A = {
    cats: () => openCategories(),
    ...
    mount(sh) {
      ...
      const pad = createLockpad({
        ...
        onComplete: async code => {
          ...
          else { first = ''; pad.reset(); hint.textContent = t('pin.mismatch'); haptic('error'); }
        }
      });
    }
  });
...
const BK_ACC_SCHEMA = V.object({
  ...
  start: V.number().min(-1e9).max(1e9).round2().optional(),
  initial: V.number().min(-1e9).max(1e9).round2().optional(),
...
const BK_CAT_SCHEMA = V.object({
  ...
  order: V.number().integer().min(0).max(9999).default(0),
  hidden: V.boolean().default(false),
  hiddenFromOverview: V.boolean().default(false)
});
...
const bkIcon = (v, fb) => {
  ...
  if (!/^[\w-]+$/.test(s)) return fb;
  if (typeof resolveLucide === 'function') return resolveLucide(s, fb);
  if (typeof LUCIDE_SVG !== 'undefined' && LUCIDE_SVG[s]) return s;
  if (typeof ICON_MIGRATION_MAP !== 'undefined' && ICON_MIGRATION_MAP[s]) return ICON_MIGRATION_MAP[s];
  return fb;
};
  ...
  if (m) {
    ...
    if (!isNaN(d.getTime()) && d.getFullYear() === +m[1] && d.getMonth() === +m[2] - 1 && d.getDate() === +m[3]) {
      ...
      if (m[4] !== undefined && m[5] !== undefined) {
        return ymd + 'T' + m[4] + ':' + m[5] + (m[6] !== undefined ? ':' + m[6] : ':00');
      }
      return ymd;
    }
  }
  return fb ? (iso(fb) + 'T12:00:00') : null;
}
...
function bkSanAccount(x, i) {
  x = x || {};
  const st = (x.start !== undefined && x.start !== null) ? x.start : ((x.initial !== undefined && x.initial !== null) ? x.initial : 0);
  ...
  return {
    ...
    initial: round2(st),
    inTotal: x.inTotal !== undefined ? (x.inTotal !== false) : !hidden,
    hidden: hidden,
...
function bkSanCat(x, i) {
  ...
  const hidden = !!(x.hidden || x.hiddenFromOverview);
  return {
    id: String(x.id || uid()),
...
function bkSanGoal(x, i) {
  ...
  return {
    ...
    saved: round2(x.saved),
    deadline: bkDate(x.deadline, null),
    color: normalizeHex(x.color) || PALETTE[0],
    icon: bkIcon(x.icon, 'target'),
    order: isFinite(+x.order) ? +x.order : i,
    history: Array.isArray(x.history) ? x.history.filter(h => h && isFinite(+h.v)).map(h => ({ date: bkDate(h.date, new Date()) || iso(new Date()), v: round2(h.v) })) : []
  };
}
...
function bkSanOp(x, i, create) {
  ...
  const date = bkOpDate(x.date, new Date()) || (iso(new Date()) + 'T12:00:00');
  return {
    id: String(x.id || uid()),
    type: type,
    amount: round2(x.amount),
...
function doExport() {
  const hadKey = !!(S.settings.ai && S.settings.ai.key);
  const now = new Date();
  const payload = {
    app: 'ONYX',
    version: 1,
  ...
  if (hadKey) setTimeout(() => toast('Ключ ИИ не включён в бэкап по соображениям безопасности'), 250);
  const r = nav[nav.length - 1]; if (r && r.id === 'settings') r.refresh();
}
window.exportData = doExport;
...
async function bkImport(src) {
  ...
  const mode = await new Promise(res => {
    ...
    openSheet({
      ...
      mount(sh) { sh.querySelectorAll('[data-m]').forEach(b => b.onclick = () => { done = true; haptic('light'); res(b.dataset.m); closeSheet(); }); },
      onClose() { if (!done) res(''); }
    });
  });
  if (!mode) return;
  ...
  try {
    ...
    S.settings.demo = false;
    S.transactions.sort(byDateDesc);
    ensureSysAccounts();
    save(); UI.accId = null;
    while (nav.length) popScreen();
