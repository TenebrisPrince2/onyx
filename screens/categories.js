...
function catsHTML() {
  ...
  return '<div class="shead"><button class="iconbtn" data-act="close"><i data-lucide="x" class="ic"></i></button><h2>Категории</h2><span class="spacer"></span>' +
    ...
    '<div style="padding:4px 14px 10px;max-width:300px;margin:0 auto;width:100%">' + segHTML('kind', kind, [['income', 'Доход', ARR_IN], ['expense', 'Расход', ARR_OUT]]) + '</div>' +
    ...
    tops.map(c => {
      ...
      return '<div class="sw" data-id="' + c.id + '">' +
        '<div class="sw__acts sw__acts--full"><button class="sw__del" data-act="del-cat" data-id="' + c.id + '" aria-label="Удалить категорию"><i data-lucide="trash" class="ic"></i><span>Удалить</span></button></div>' +
...
const CategoryFormScreen = (function () {
  ...
  const st = { type: 'expense', name: '', icon: 'star', color: '#ff9500', parentId: null, customOpen: false, hidden: false };
  ...
  function open(params, done) {
    ...
    if (params.id) {
      ...
      if (c) {
        ...
        st.hidden = !!(c.hidden || c.hiddenFromOverview);
      }
    ...
    fsEl.innerHTML =
      ...
      '<div class="fs-body">' +
        ...
        '<div class="fs-sec fs-sec__hide" style="margin-top:16px">' +
          '<button class="acc-hide-card" id="category-form-hide-toggle" type="button" role="switch" aria-checked="' + (st.hidden ? 'true' : 'false') + '" aria-labelledby="cat-hide-title" aria-describedby="cat-hide-desc">' +
            '<span class="acc-hide-card__ic">' + icon('hat-glasses', 18) + '</span>' +
            '<span class="acc-hide-card__text">' +
              '<span class="acc-hide-card__title" id="cat-hide-title">Скрыть из обзора</span>' +
              '<span class="acc-hide-card__desc" id="cat-hide-desc">Категория остаётся в списке, но<br>исключается из главного обзора.</span>' +
            '</span>' +
  ...
  function renderPicker() {
    ...
    var html = '<div class="ipick-wrap">' + (cats ? cats.map(function (cat) {
        ...
        cat.rows.map(function (row) {
          return '<div class="ipick-row">' +
            row.map(function (n) {
              ...
              var on = st.icon === n;
              return '<button class="ipick-btn' + (on ? ' on' : '') + '" data-i="' + n + '" type="button" aria-label="' + esc(n) + '" aria-pressed="' + on + '">' + icon(n, 20) + '</button>';
            }).join('') +
          '</div>';
        }).join('') +
      '</div></div>';
      ...
      return '<div class="ipick-sec"><div class="ipick-t">' + esc(label) + '</div><div class="ipick-rows"><div class="ipick-row">' +
  ...
  function bind() {
    ...
    var hideBtn = $id('category-form-hide-toggle');
    if (hideBtn) {
  ...
  function openParentSheet() {
    ...
    const html = '<div class="list">' +
      pRow('', '<span class="tile tile--sm" style="--c:var(--t3)"><i data-lucide="circle-slash" class="ic"></i></span>', 'Без родителя', noneOn) +
      opts.map(function (c) {
