...
function CenterCarousel(track, ops) {
  ...
  function paint() {
    ...
    if (near !== lastNear) { lastNear = near; caption(near); measure(); }
  }
  ...
  function emit(i) {
    ...
    if (raw === '__none') { haptic('selection'); ops.onChange(null); return; }
    if (raw.charAt(0) === '_') { haptic('light'); (ops.onAction || ops.onAdd || function () {})(raw, i); return; }
    haptic('selection');
    ops.onChange(raw);
  }
  function snap() {
  ...
  track.addEventListener('click', e => {
    ...
    if (raw.charAt(0) === '_' && raw !== '__none') {
      if (i === lastEmit) { haptic('light'); (ops.onAction || ops.onAdd || function () {})(raw, i); return; }
      emit(i);
      return;
    }
    const target = i * STEP;
...
function bindCatPager(el, opts) {
  ...
  function rebuildKids() {
    recenterKids();
    if (state === 'kids' && !hasKidsOf(parentSel)) { state = 'top'; }
    applyState();
  }
  function setState(s) {
...
function edHTML() {
  ...
  const edTime = (edTxn && edTxn.date.slice(11) ? edTxn.date.slice(11) : new Date().toTimeString().slice(0, 5)).slice(0, 5);
  const trDate = '<i data-lucide="archive" class="ic ic-s"></i>' + (ED.date === iso(new Date()) ? 'Сегодня, ' + edTime : shortDate(ED.date) + ', ' + edTime);
...
function edMount(el) {
  ...
  el.querySelectorAll('.key').forEach(k => {
    ...
    const press = () => {
      const now = Date.now();
      if (now - lastPress < 60) return;
      lastPress = now;
      const key = k.dataset.k; haptic('selection');
    ...
    k.onclick = press;
    k.addEventListener('pointerdown', () => { if (k.dataset.k === 'bs') lp = setTimeout(() => { ED.entry = '0'; ED.left = null; ED.op = null; haptic('medium'); redrawAmt(); }, 480); });
  ...
  el.querySelectorAll('[data-act]').forEach(b => {
    ...
    if (a === 'tr-swap') b.onclick = () => { haptic('medium'); const sw = ED.accountId; ED.accountId = ED.toAccountId; ED.toAccountId = sw; patch(); };
    ...
    if (a === 'ed-cur') b.onclick = () => { haptic('selection'); ED.currency = CUR_ORDER[(CUR_ORDER.indexOf(ED.currency) + 1) % CUR_ORDER.length]; patch(); };
...
function edMore() {
  const items = [];
  if (ED.type === 'income') items.push(['sparkle', ED.spVisible ? 'Скрыть умный план' : 'Умный план', () => { ED.spVisible = !ED.spVisible; ED.rec.refresh(); }]);
  ...
  if (ED.id) items.push(['trash', 'Удалить операцию', async () => { closeSheet(); if (await confirmSheet({ title: 'Удалить операцию?', text: 'Действие можно отменить сразу после удаления.', ok: 'Удалить', danger: true })) { delTxn(ED.id); popScreen(); } }]);
  openSheet({
    title: 'Операция',
...
function saveEditor() {
  ...
  const t = {
    ...
    note: ED.note.trim(),
    date: ED.date + 'T' + nowTime,
    createdAt: nowIso
  };
  if (ED.id) {
    const existing = S.transactions.find(x => x.id === ED.id);
    if (existing) {
      const prevTime = (existing.date && existing.date.slice(11)) ? existing.date.slice(11) : nowTime;
      t.date = ED.date + 'T' + prevTime;
      t.createdAt = existing.createdAt || nowIso;
    }
    Store.updateTransaction(ED.id, t);
  } else {
    Store.addTransaction(t);
  }
  /* fresh-подсветка: ledger render() подхватит id и мягко подсветит строку */
  try { if (typeof sessionStorage !== 'undefined') sessionStorage.setItem('onyx:freshTxn', t.id); } catch (e) {}
  haptic('success'); popScreen();
...
async function delTxn(id) {
  ...
  const icon = t ? (t.type==='transfer'?'arrow-left-right': t.type==='income' ? 'chart-candlestick' : (cat ? catLuc(cat.icon) : 'receipt')) : 'trash';
  ...
  toast('Удалено', { label: 'Вернуть', fn: () => { Store.restoreTransaction(removed, i); render(true); refreshTop(); toast('Операция восстановлена'); } }, { icon: 'trash' });
}
