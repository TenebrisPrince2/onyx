...
function accHTML() {
  ...
  return '<div class="shead"><button class="iconbtn" data-act="close"><i data-lucide="x" class="ic"></i></button><h2>Счета</h2><span class="spacer"></span>' +
    ...
    '<div id="accList" style="display:flex;flex-direction:column;gap:2px">' +
    list.map(a => a.system
      ...
      '<div class="sw__acts sw__acts--full"><button class="sw__del" data-act="del-acc" data-id="' + a.id + '" aria-label="Удалить счёт"><i data-lucide="trash" class="ic"></i><span>Удалить</span></button></div>' +
      '<div class="sw__body" data-act="pick-acc" data-id="' + a.id + '" aria-selected="' + (UI.accId === a.id) + '" style="cursor:pointer">' +
      '<span class="tile" style="--c:' + a.color + '"><i data-lucide="' + a.icon + '" class="ic"></i></span>' +
    ...
    '<p style="color:var(--t3);font-size:13px;line-height:1.55;padding:18px 6px 0"></p>' +
    '</div>';
}
function accMount(el) {
  ...
  if (pickAll) pickAll.onclick = () => { UI.accId = null; UI.ledger.day = null; UI.ledger.limit = 50; haptic('selection'); render(); popScreen(); refreshTop(); };
  el.querySelectorAll('[data-act="pick-acc"]').forEach(b => b.onclick = e => { if (b.dataset.swiped === '1') return; if (e.target.closest('[data-act="edit-acc"]') || e.target.closest('[data-handle]')) return; UI.accId = b.dataset.id; UI.ledger.day = null; UI.ledger.limit = 50; haptic('selection'); render(); popScreen(); refreshTop(); });
...
const AccountFormScreen = (function () {
  ...
  const st = { name: '', icon: 'wallet', color: '#10b981', start: '0', currency: 'BYN', customOpen: false, hidden: false };
  let editing = null, onDone = null, fsEl = null, fsScrim = null, bsEl = null, bsOpen = false;
  ...
  function open(params, done) {
    ...
    if (fsScrim) { fsScrim.remove(); fsScrim = null; }
    fsEl = null;
    st.customOpen = false;
    ...
    const d = editing;
    fsScrim = document.createElement('div');
    fsScrim.className = 'scrim';
    ...
    fsEl.innerHTML =
      ...
      '<div class="fs-body fs-body--sec">' +
        ...
        '<div class="fs-sec fs-sec__bal"><div class="bal-card">' +
            '<div class="bal-row">' +
              '<span class="bal-ic">' + icHtml('landmark', 18) + '</span>' +
          ...
          '</div>' +
        '</div>' +
        '<div class="fs-sec fs-sec__hide">' +
          '<button class="acc-hide-card" id="account-form-hide-toggle" type="button" role="switch" aria-checked="' + (st.hidden ? 'true' : 'false') + '" aria-labelledby="acc-hide-title" aria-describedby="acc-hide-desc">' +
            '<span class="acc-hide-card__ic">' + icHtml('hat-glasses', 18) + '</span>' +
            '<span class="acc-hide-card__text">' +
              '<span class="acc-hide-card__title" id="acc-hide-title">Скрыть из обзора</span>' +
              '<span class="acc-hide-card__desc" id="acc-hide-desc">Счёт остаётся в списке, но<br>исключается из общей «Всего счетов».</span>' +
            '</span>' +
            '<span class="acc-hide-card__switch" aria-hidden="true">' +
              '<span class="acc-hide-card__thumb"></span>' +
            '</span>' +
          '</button>' +
        '</div>' +
    ...
    $('#overlays').append(fsScrim, fsEl);
    renderPalette();
    renderPicker();
    bind();
    icons(fsEl);   /* PERF: вся форма обрабатывается один раз; внутренние перерисовки скоупятся к своим контейнерам */
    fsScrim.onclick = close;
    void fsEl.offsetHeight;
    requestAnimationFrame(() => {
      if (fsScrim) fsScrim.classList.add('in');
      if (fsEl) fsEl.classList.add('show');
    });
  }
  function close() {
    if (bsEl) { bsEl.remove(); bsEl = null; bsOpen = false; }
    if (!fsEl) return;
    const el = fsEl, scrim = fsScrim;
    fsEl = null; fsScrim = null;
  ...
  function renderPicker() {
    var cats = (typeof ICON_CATEGORIES !== 'undefined' && ICON_CATEGORIES) ? ICON_CATEGORIES : null;
    var html = '<div class="ipick-wrap">' + (cats ? cats.map(function (cat) {
      return '<div class="ipick-sec"><div class="ipick-t">' + esc(cat.name) + '</div><div class="ipick-rows">' +
        cat.rows.map(function (row) {
          return '<div class="ipick-row">' +
            row.map(function (n) {
              if (!n) return '<div class="ipick-empty" aria-hidden="true"></div>';
              var on = n === st.icon;
              return '<button class="ipick-btn' + (on ? ' on' : '') + '" data-i="' + n + '" type="button" aria-label="' + esc(n) + '" aria-pressed="' + on + '">' + icHtml(n, 20) + '</button>';
            }).join('') +
          '</div>';
        }).join('') +
      '</div></div>';
    }).join('') : SECTIONS.map(function (sec) {
      var names = sec[1];
      return '<div class="ipick-sec"><div class="ipick-t">' + esc(sec[0]) + '</div><div class="ipick-rows"><div class="ipick-row">' +
  ...
  function bind() {
    ...
    var hideBtn = $id('account-form-hide-toggle');
    if (hideBtn) {
      hideBtn.onclick = function () {
        st.hidden = !st.hidden;
        hideBtn.setAttribute('aria-checked', st.hidden ? 'true' : 'false');
        haptic('selection');
      };
    }
    ...
    const grab = fsEl ? fsEl.querySelector('.grabber') : null;
    if (grab && typeof dragDismiss === 'function') dragDismiss(fsEl, grab, close);
  }
  function openBs(inner) {
    ...
    const panel = bsEl.querySelector('.bs-panel');
    panel.innerHTML = '<div class="grabber"></div>' + inner;
    const pGrab = panel.querySelector('.grabber');
    if (pGrab && typeof dragDismiss === 'function') dragDismiss(panel, pGrab, closeBs);
  ...
  function closeBs() {
    ...
    bsOpen = false;
    const host = bsEl;
    setTimeout(() => { host.setAttribute('aria-hidden', 'true'); const p = host.querySelector('.bs-panel'); if (p) p.innerHTML = ''; }, 280);
  }
  ...
  function saveForm() {
    ...
    if (editing) {
      ...
      if (typeof Store !== 'undefined' && Store.updateAccount) {
        saved = Store.updateAccount(editing.id, payload);
      } else {
      ...
      if (typeof Store !== 'undefined' && Store.addAccount) {
        saved = Store.addAccount(payload);
      } else {
