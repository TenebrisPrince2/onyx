...
const DEF_CATS = [
  ...
  ['Здоровье', 'activity', 5, 'expense'], ['Спорт', 'dumbbell', 7, 'expense'],
  ['Транспорт', 'car', 6, 'expense'], ['Жильё и связь', 'house', 11, 'expense'],
...
function fresh() {
  ...
  st.goals = [
    ...
    { id: 'g1', name: 'Отпуск, Тбилиси', target: 3400, saved: 1980, icon: 'plane', color: PALETTE[6], deadline: iso(addM(new Date(), 3)), history: [] }
  ];
...
function demoTxns(st) {
  ...
  const rndAmt = (a, b) => round2(a + rnd() * (b - a));
  const rows = [
  ...
  for (let i = 62; i >= 0; i--) {
    ...
    rows.forEach(r => {
      if (rnd() < r[5] * (d.getDay() === 0 || d.getDay() === 6 ? 1.25 : .9)) {
        out.push({ id: uid(), type: 'expense', amount: rndAmt(r[2], r[3]), accountId: r[1], categoryId: r[0], note: pick(r[4]), date: k + 'T' + String(8 + Math.floor(rnd() * 13)).padStart(2, '0') + ':' + String(Math.floor(rnd() * 60)).padStart(2, '0') });
      }
    });
    ...
    if (d.getDate() % 7 === 3 && i > 3) out.push({ id: uid(), type: 'transfer', amount: rndAmt(30, 120), accountId: 'a0', toAccountId: 'a1', note: 'Внести наличные', date: k + 'T19:45' });
    if (d.getDate() === 8 && i < 45) out.push({ id: uid(), type: 'transfer', amount: rndAmt(50, 250), accountId: 'a1', toAccountId: 'a2', note: 'На накопления', date: k + 'T12:05' });
    if (d.getDate() === 20 && i < 45) out.push({ id: uid(), type: 'income', amount: rndAmt(25, 90), accountId: 'a2', categoryId: 'c12', note: 'Возврат долга', date: k + 'T14:15' });
    if (d.getDate() === 17 && i < 30) out.push({ id: uid(), type: 'income', amount: rndAmt(15, 60), accountId: 'a1', categoryId: 'c12', note: 'Кэшбэк и проценты', date: k + 'T10:25' });
  }
