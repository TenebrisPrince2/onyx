...
async function load() {
  ...
  if (typeof invalidateLedgerCache === 'function') invalidateLedgerCache();
}

function migrateIcons(st) {
  if (!st) return false;
  const mig = id => (typeof ICON_MIGRATION_MAP !== 'undefined' && ICON_MIGRATION_MAP[id]) ? ICON_MIGRATION_MAP[id] : id;
  let changed = false;
  ['accounts', 'categories', 'goals'].forEach(key => {
    if (Array.isArray(st[key])) {
      st[key].forEach(item => {
        if (item && item.icon) {
          const next = mig(item.icon);
          if (next !== item.icon) {
            item.icon = next;
            changed = true;
          }
...
function persistMirror(force) {
  ...
  lastMirror = now;
  try { const raw = localStorage.getItem(KEY); if (raw) localStorage.setItem(KEY_BAK, raw); } catch (e) { console.warn('[persist]', e); }
}
function persist(opts) {
  try {
    ...
    try { idbPut(S); } catch (e) { console.warn('[persist]', e); }        // внешний бэкап в IndexedDB — переживает очистку localStorage
  } catch (e) {
    toast('Не удалось сохранить: хранилище переполнено', null, { tone: 'danger' });
  }
  if (!(opts && opts.silent) && typeof emitStore === 'function') emitStore();
}
