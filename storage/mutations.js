...
const Store = {
  ...
  addTransaction(t) {
    ...
    if (typeof txnSortCache !== 'undefined' && txnSortCache.invalidate) txnSortCache.invalidate();
    save();
    return rec;
  },
  updateTransaction(id, patch) {
    ...
    if (!t) return null;
    if (patch.amount !== undefined) patch.amount = Math.max(0, round2(patch.amount));
  ...
  addAccount(a) {
    ...
    const inTotal = a.inTotal !== undefined ? a.inTotal : !hidden;
    const rec = Object.assign({ id: a.id || uid(), name: '', currency: S.settings.currency, initial: 0, inTotal: inTotal, hidden: hidden, order: S.accounts.length, archived: false }, a);
    S.accounts.push(rec);
    save();
    return rec;
  },
  updateAccount(id, patch) {
    ...
    if (patch.hidden !== undefined && patch.inTotal === undefined) {
      patch.inTotal = !patch.hidden;
    }
  ...
  upsertCategory(c) {
    ...
    if (data.hidden !== undefined && data.hiddenFromOverview === undefined) {
      data.hiddenFromOverview = !!data.hidden;
    } else if (data.hiddenFromOverview !== undefined && data.hidden === undefined) {
      data.hidden = !!data.hiddenFromOverview;
    }
    let cat = data.id ? S.categories.find(x => x.id === data.id) : null;
    if (cat) {
      Object.assign(cat, data);
    } else {
      const isHidden = data.hidden === true || data.hiddenFromOverview === true;
      cat = Object.assign({
        id: uid(),
        name: '',
        kind: 'expense',
  ...
  addCategory(c) {
    return this.upsertCategory(Object.assign({}, c, { id: (c && c.id) || uid() }));
  },
  updateCategory(id, patch) {
    return this.upsertCategory(Object.assign({}, patch, { id: id }));
  },
  ...
  commit(fn) {
    fn(S);
    if (Array.isArray(S.transactions)) S.transactions.sort(byDateDesc);
    if (typeof txnSortCache !== 'undefined' && txnSortCache.invalidate) txnSortCache.invalidate();
    save();
  }
};
